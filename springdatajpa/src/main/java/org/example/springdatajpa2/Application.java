...
//고객정보와 주문수 출력!!
            ...
            // 고객 정보와 최근 주문 출력
            List<Object[]> customersWithLatestOrder = repository.findCustomersWithLatestOrder();
            for (Object[] latest: customersWithLatestOrder) {
                Customer customer = (Customer) latest[0];
                Order order = (Order) latest[1];
                System.out.println("고객: " + customer.getName() + "최근주문날짜: " + order.getDate());
            }
