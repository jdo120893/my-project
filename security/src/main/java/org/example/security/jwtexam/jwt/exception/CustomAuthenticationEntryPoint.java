package org.example.security.jwtexam.jwt.exception;
...
//            rest 요청이 들어왔을 때의 오류처리!!
    ...
    private void handleRestResponse(String exeception, HttpServletRequest request,
                                    HttpServletResponse response) throws IOException, ServletException{
        log.error("Page Request - commence : {}",exeception);

        JwtExceptionCode code = JwtExceptionCode.findByCode(exeception);
        if(code == JwtExceptionCode.UNKNOWN_ERROR && exeception == null){
            //이렇게 처리할 거예요.
