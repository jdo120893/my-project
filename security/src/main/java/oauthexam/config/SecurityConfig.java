package oauthexam.config;

import lombok.Builder;
import lombok.RequiredArgsConstructor;
import oauthexam.security.CustomOAuth2AuthenticationSuccessHandler;
import oauthexam.service.SocialUserService;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http,CorsConfigurationSource configurationSource)throws Exception{
        http
                .authorizeHttpRequests(auth->auth
                        .requestMatchers("/", "/userregform", "/loginform", "/loginFailure").permitAll()
                        .requestMatchers("/oauth2/**", "/login/oauth2/code/github",
                                "/saveSocialUser", "/registerSocialUser").permitAll()
                        .requestMatchers("/welcome").authenticated()  // 로그인한 사용자만
